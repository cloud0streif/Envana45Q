...
} from '../lib/mock-data/injector-data'
...
export function InjectorDashboard({ wellId: wellIdProp, embedded = false }: InjectorDashboardProps = {}) {
  ...
  const chartData = useMemo(() => {
    ...
    const timeSeriesData = generateMockTimeSeries(days, parseInt(wellId))

    // Format data for Recharts
    return timeSeriesData.map((point) => ({
      timestamp: point.timestamp,
      displayTime: format(new Date(point.timestamp), days <= 1 ? 'HH:mm' : days <= 30 ? 'MMM dd' : 'MMM yyyy'),
      co2_mass: Number(point.co2_mass_injected.toFixed(1)),
      density: Number(point.fluid_density.toFixed(3)),
      composition: Number(point.co2_composition.toFixed(1)),
      pressure: Number(point.injection_pressure.toFixed(1)),
      temperature: Number(point.injection_temperature.toFixed(1)),
    }))
  }, [selectedPeriod, customStartDate, customEndDate, wellId])

  // Calculate summary statistics from chart data
  const calculatedMeasurements = useMemo(() => {
    if (chartData.length === 0) return mockMeasurements

    const totalMass = chartData.reduce((sum, point) => sum + point.co2_mass, 0)
    ...
    return {
      co2_mass_injected: {
        ...mockMeasurements.co2_mass_injected,
        value: Number(totalMass.toFixed(1)),
      },
      fluid_density: {
        ...mockMeasurements.fluid_density,
        value: Number(avgDensity.toFixed(3)),
      },
      co2_composition: {
        ...mockMeasurements.co2_composition,
        value: Number(avgComposition.toFixed(1)),
      },
      injection_pressure: {
        ...mockMeasurements.injection_pressure,
        value: Number(avgPressure.toFixed(1)),
      },
      injection_temperature: {
        ...mockMeasurements.injection_temperature,
  ...
  const getDependentDocuments = (measurementKey: string) => {
    const requiredDocTypes = MEASUREMENT_DEPENDENCIES[measurementKey] || []
  ...
  const currentDocuments = useMemo(() => {
    const docsByType: Record<string, ComplianceDocument> = {}
          ...
          >
            ...
            <p className="text-3xl font-bold text-gray-900 mb-3">
              {measurement.value >= 1000
                ...
                : measurement.unit === 'g/cm³' ? measurement.value.toFixed(2) : measurement.value.toFixed(1)
              } {measurement.unit}
            </p>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <YAxis
              yAxisId="left"
              tick={{ fontSize: 12 }}
              label={{ value: 't / bar / °C', angle: -90, position: 'insideLeft' }}
            />
            <YAxis
              yAxisId="right"
              orientation="right"
              tick={{ fontSize: 12 }}
              label={{ value: 'g/cm³ / % purity', angle: 90, position: 'insideRight' }}
            />
            <Tooltip
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="font-semibold text-gray-700 mb-2">Wellbore Integrity, Pressure Falloff Test, Coriolis Meter:</p>
              <ul className="list-disc list-inside text-gray-600 space-y-1">
                <li>CO2 Mass Injected</li>
                <li>Fluid Density</li>
              </ul>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="font-semibold text-gray-700 mb-2">Wellbore Integrity, Pressure Falloff Test, Gas Analyzer:</p>
              <ul className="list-disc list-inside text-gray-600 space-y-1">
                <li>CO2 Composition</li>
              </ul>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="font-semibold text-gray-700 mb-2">Wellbore Integrity, Pressure Falloff Test, P&T Transducer:</p>
              <ul className="list-disc list-inside text-gray-600 space-y-1">
                <li>Injection Pressure</li>
                <li>Injection Temperature</li>
              </ul>
            </div>
          </div>
        </div>
